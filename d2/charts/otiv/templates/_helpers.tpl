...
{{/*
Outputs the yaml with the java cacert password 
Placed within the env section of the deployment descriptor
Template only outputs if amqp is configured for ssl
*/}}
{{- define "java.store.secrets" -}}
{{- if .Values.global.amqp.ssl -}}
{{- if .Values.global.cssFqdn -}}
          - name: AJIRA_KEYSTORE_PASSWORD
            value: 
          - name: AJIRA_TRUSTSTORE_PASSWORD
            value: 
{{- else -}}
          - name: AJIRA_KEYSTORE_PASSWORD
            valueFrom:
              secretKeyRef:
                name: {{ include "iv-cvt.product.prefix" . }}-base-secrets
                key: javact
          - name: AJIRA_TRUSTSTORE_PASSWORD
            valueFrom:
              secretKeyRef:
...
Template only outputs if amqp is configured for ssl
*/}}
{{- define "rb.notify.secrets" -}}
{{- if and .Values.global.amqp.ssl .Values.global.cssFqdn -}}
          - name: NOTIFY_RMQ_KEYSTORE_PASSWORD
            value: 
          - name: NOTIFY_RMQ_TRUSTSTORE_PASSWORD
            value: 
{{- end -}}
{{- end -}}
